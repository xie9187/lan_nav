...
class DDPG(object):
    ...
    def ActorPredict(self, 
                    ...
                    input_goal, 
                    prev_state_2):
        a, state_2 = self.actor.PredictOnline(input_laser, 
                                             input_cmd, 
                                             input_cmd_next, 
                                             input_cmd_skip, 
                                             prev_action, 
    ...
    def Add2Mem(self, sample):
        if len(sample) <= self.max_step:
            self.memory.append(sample) # seqs of (laser, cmd, cmd_next, cmd_skip,
    ...
    def SampleBatch(self):
        if len(self.memory) >= self.batch_size:
            ...
            return [laser_t_batch, cmd_t_batch, cmd_next_t_batch, cmd_skip_t_batch, prev_action_t_batch, 
                    goal_a_t_batch, goal_a_t_batch, prev_state_2_t_batch, action_t_batch,
                    reward_batch, terminate_batch, status_batch, action_batch,
                    laser_t1_batch, cmd_t1_batch, cmd_next_t1_batch, action_t_batch, 
                    goal_a_t1_batch, goal_a_t1_batch, action_t1_batch], indices
        else:
    ...
    def Train(self):
        ...
        if not batch:
            ...
            [laser_t_batch, cmd_t_batch, cmd_next_t_batch, cmd_skip_t_batch, prev_action_t_batch, 
             goal_t_batch, goal_a_t_batch, prev_state_2_t_batch, action_t_batch,
             reward_batch, terminate_batch, status_batch, action_batch,
             laser_t1_batch, cmd_t1_batch, cmd_next_t1_batch, prev_action_t1_batch, 
             goal_t1_batch, goal_a_t1_batch, action_t1_batch] = batch
            ...
            actions, states_2 = self.actor.PredictOnline(laser=laser_t_batch, 
                                                                 cmd=cmd_t_batch, 
                                                                 cmd_next=cmd_next_t_batch, 
